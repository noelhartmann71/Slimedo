...
const PharmacyDashboardLogin = () => {
  ...
  const loginMutation = useMutation({
    mutationFn: async (userData: LoginFormValues) => {
      const response = await axiosPublic.post("/pharmacy/login", userData);
      return response;
    },
    onSuccess: (response) => {
      ...
      form.reset();
      navigate("/pharmacy-dashboard/overview");
    },
    onError: (error: AxiosError<{ message?: string }>) => {
      const errorMessage =
        error?.response?.data?.message || (error as Error).message;
      console.error("Pharmacy Login failed:", errorMessage);
          ...
          <div>
            ...
            <div className="space-y-4">
              <h1 className="text-[32px] font-medium leading-tight">
                Medizinisch geprüfte Online‑Rezepte
              </h1>
              <p className="text-base leading-relaxed opacity-90">
                Slimedo unterstützt Patienten und Ärzte bei nahtloser Kommunikation.
                Termine buchen, Aufzeichnungen verfolgen und Ihre Gesundheitsreise verbessern.
              </p>
            </div>
          </div>
...
export default PharmacyDashboardLogin;
